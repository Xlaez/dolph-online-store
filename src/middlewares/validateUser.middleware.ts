...
const validateAcc = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof token !== 'string') return next(new AppRes(httpStatus.UNAUTHORIZED, 'provide a valid token type'));
  ...
  try {
    ...
    if (!user.isAccountValid) return next(new AppRes(httpStatus.UNAUTHORIZED, 'validate your account'));
